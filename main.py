...
# 初始化区域列表，共31个区域（Azure for Students和即用即付订阅均不支持 South India 和 West India 区域）
locations = ['eastus', 'eastus2', 'westus', 'centralus', 'northcentralus', 'southcentralus', 'northeurope',
             'westeurope', 'eastasia', 'southeastasia', 'japaneast', 'japanwest', 'australiaeast', 'australiasoutheast',
             'australiacentral', 'brazilsouth', 'centralindia', 'canadacentral', 'canadaeast', 'westus2', 'uksouth',
             'ukwest', 'koreacentral', 'koreasouth', 'francecentral', 'southafricanorth', 'uaenorth',
             'switzerlandnorth', 'germanywestcentral', 'norwayeast', 'westcentralus']
...
# Azure for Students订阅每个区域的vCPU总数为6，标准FSv2系列vCPUs为4，标准FS系列vCPUs为4
# 所以创建一个Standard_F4s_v2实例（占用4个vCPUs），一个Standard_F2s实例（占用2个vCPUs）
if '6' in limit:
...
# 因此创建一个Standard_F8s_v2实例（占用8个vCPUs），一个Standard_F2s_v2实例（占用2个vCPUs）
elif '10' in limit:
    print("当前订阅为即用即付")
...
# 因此创建1个Standard_F4s_v2实例（共占用4个vCPUs）
    ...
    print("当前订阅为免费试用，每个区域的配额仅为4 vCPUs，建议升级后再用。若升级后仍看到本消息，请等待十分钟再运行脚本。")
...
for location in locations:
    ...
    # westcentralus 区域不支持 FSv2 系列，Azure for Students订阅不支持 F/FS 系列
    ...
    for a in range(0, size1_count):
        count += 1
        print("正在 " + str(location) + " 区域创建第 " + str(count) + f" 个 {size1_name} 实例，共 " + str(size1_count) + " 个")
        get_default_cli().invoke(
            ['vm', 'create', '--resource-group', 'myResourceGroup', '--name',
             f'{location}-{size1_abbreviation}-{count}',
             '--image', 'UbuntuLTS', '--size', f'{size1_name}', '--location', f'{location}', '--admin-username',
             'azureuser', '--admin-password', '6uPF5Cofvyjcew9', '--custom-data', 'cloud-init.txt', "--no-wait"])
    if type != 2:
        count = 0
        for a in range(0, size2_count):
            count += 1
            print("正在 " + str(location) + " 区域创建第 " + str(count) + f" 个 {size2_name} 实例，共 " + str(size2_count) + " 个")
            get_default_cli().invoke(
                ['vm', 'create', '--resource-group', 'myResourceGroup', '--name',
                 f'{location}-{size2_abbreviation}-{count}',
                 '--image', 'UbuntuLTS', '--size', f'{size2_name}', '--location', f'{location}', '--admin-username',
                 'azureuser', '--admin-password', '6uPF5Cofvyjcew9', '--custom-data', 'cloud-init.txt', "--no-wait"])
